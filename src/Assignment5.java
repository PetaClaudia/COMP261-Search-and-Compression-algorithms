...
public class Assignment5 {
	...
	// editor components.
	private JTextField kmpSearchField;
	...
	private void onAlgorithmRun() {
		try {
			...
			if (algorithm.equals("Huffman coding")) {
				...
				float difference = compressionInputFile.length()-(encoded.length() / 8);
				compressionOutput.append("reduction: " + (difference/compressionInputFile.length())*100 +"%\n \n");
	...
	private void initialise() {
		...
		kmpSearchField = new JTextField(SEARCH_COLS);
		kmpSearchField.setMaximumSize(new Dimension(0, 25));
		kmpSearchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String pattern = kmpSearchField.getText();
				String text = textEditor.getText();
				int index = new KMP(pattern, text).search(pattern, text);
				//int index = new BruteForceSearch(pattern, text).search(pattern, text);
		...
		controls.add(new JLabel("KMP Search"));
		controls.add(Box.createRigidArea(new Dimension(LAYOUT_GAP, 0)));
		controls.add(kmpSearchField);
